...
(defn validate-player-biases
  ...
  [players]
  (let [invalid-players
        (filter (fn [{:keys [name attacking-bias defensive-bias]}]
...
(defn validate-shooting-railway
  ...
  [match-data]
  (let [invalid-matches
        (filter (fn [{:keys [match-id team-colours-data team-bibs-data]}]
  ...
  (let [bias-validation (validate-player-biases player-data)
    ...
    (if all-valid?
      ...
      (doseq [[validation-type result]
        ...
        (when (and (seq result) (not (empty? result)))
          ...
          (cond
            ...
            :else
            (println (str "  - " (clojure.string/join ", " result)))))))))